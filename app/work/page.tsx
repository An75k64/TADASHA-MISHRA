...
const areaIcons = [
  (
    <svg key="area-icon-1" viewBox="0 0 24 24" className="h-6 w-6 text-[#e8850a]" fill="none" stroke="currentColor" strokeWidth="2">
      <circle cx="10" cy="10" r="7" />
      <path d="M15 15l5 5" />
    </svg>
  ),
  (
    <svg key="area-icon-2" viewBox="0 0 24 24" className="h-6 w-6 text-[#e8850a]" fill="none" stroke="currentColor" strokeWidth="2">
      <path d="M12 3l4 4-1.4 1.4L12 6.8 9.4 8.4 8 7 12 3z" />
      <path d="M4 11h16v10H4z" />
    </svg>
  ),
  (
    <svg key="area-icon-3" viewBox="0 0 24 24" className="h-6 w-6 text-[#e8850a]" fill="none" stroke="currentColor" strokeWidth="2">
      <path d="M6 4l12 8-12 8V4z" />
    </svg>
  ),
  (
    <svg key="area-icon-4" viewBox="0 0 24 24" className="h-6 w-6 text-[#e8850a]" fill="none" stroke="currentColor" strokeWidth="2">
      <path d="M5 12h14" />
...
export default function WorkPage() {
  const [data, setData] = useState({
    keyAreas: defaultKeyAreas,
    highlights: defaultHighlights
  });

  useEffect(() => {
    fetch('/api/admin/settings?key=work')
      .then(res => res.json())
      .then(fetchedData => {
        if (fetchedData && fetchedData.keyAreas && fetchedData.highlights) {
          setData(fetchedData);
        }
      })
      .catch(console.error);
        ...
        <div className="mt-10 space-y-4">
          ...
          {data.keyAreas.map((area, index) => (
            ...
            >
              ...
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#f8f0e6] shadow-sm shrink-0">
